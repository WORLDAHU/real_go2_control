...
2. q_home：固定姿态的转子编码器参考（rad）
   GO-M8010-6 使用转子侧单圈绝对式编码器。在固定人工标定姿态读取的 q
   保存了绝对单圈相位参考；电机断电后固件不保留累计圈数，所以同一姿态
   的 q 可能相差整数个 2*pi。它不是输出轴多圈绝对角、电机出厂零点，
   也不是所有关节的 common 机械零位。bridge 的底层换算为：

       q_target = q_home + radians(bridge_cmd_deg * direction) * gear_ratio

   bridge 会先把 q_home 对齐到当前累计圈数分支；bridge_cmd_deg=0 的含义
   是“回到固定标定姿态”。没有多圈绝对或输出轴绝对传感器时，仍须人工
   确认机械分支。
...
大腿最容易混淆：大腿水平是固定标定姿态，不是 common thigh 的机械零位。
